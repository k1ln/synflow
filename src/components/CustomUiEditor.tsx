...
/** Modal: write a custom HTML faceplate for a flow, with a live, playable preview.
 *  The saved HTML lives on flow.customUi and round-trips to the DAW. */
export function CustomUiEditor({ title, kind, initialHtml, knobs, valueOf, onKnob, onNoteOn, onNoteOff, onHit, onSave, onClose }: {
  title: string;
  ...
  return (
    <div className="lui-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="lui-cui-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="lui-head">
          <span className="lui-dot" />
          <span className="lui-title">Custom UI — {title}</span>
          <span className="lui-hint">bind with data-param · data-note · data-readout · data-hit</span>
          <button className="lui-close" style={{ marginLeft: 'auto' }} onClick={onClose} title="Close"><X size={16} /></button>
        </div>
        <div className="lui-cui-body">
          <div className="lui-cui-edit">
            <div className="lui-cui-toolbar">
              <button className="lui-cui-tool" onClick={() => setDraft(customUiTemplate(knobs, kind))} title="Replace with a starter template generated from this flow's knobs"><Wand2 size={13} /> Template</button>
              <button className="lui-cui-tool" onClick={() => setDraft(initialHtml)} disabled={draft === initialHtml} title="Revert to the saved UI"><RotateCcw size={13} /> Revert</button>
              <span className="lui-cui-attrs">{knobs.length} param{knobs.length === 1 ? '' : 's'}: {knobs.map((k) => `${k.nodeId}.${k.param}`).join(' · ') || '—'}</span>
            </div>
            <textarea className="lui-cui-code" value={draft} spellCheck={false} onChange={(e) => setDraft(e.target.value)}
              placeholder="Write HTML here…" />
          </div>
          <div className="lui-cui-preview">
            <div className="lui-cui-preview-label">Live preview — playable</div>
            <CustomInstrumentUI className="lui-cui-preview-stage" html={draft} knobs={knobs} valueOf={valueOf}
              onKnob={onKnob} onNoteOn={onNoteOn} onNoteOff={onNoteOff} onHit={onHit} />
          </div>
        </div>
        <div className="lui-cui-foot">
          <button className="lui-btn-secondary" onClick={onClose}>Cancel</button>
          <button className="lui-btn-primary" onClick={() => { onSave(draft); onClose(); }}><Save size={14} /> Save UI</button>
        </div>
      </div>
    </div>
  );
