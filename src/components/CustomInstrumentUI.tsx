 ...
 *   • data-hit on any element — triggers a one-shot drum hit (flash `.on` briefly).
 *
 * Identical contract to the DAW's CustomInstrumentUI, so a faceplate authored in
 * Synflow renders the same in Mothscilla.
 */
export function CustomInstrumentUI({ html, knobs, valueOf, onKnob, onNoteOn, onNoteOff, onHit, className }: {
  ...
  // Octave transpose for the faceplate keyboard (semitones, multiples of 12),
  // driven by [data-octave] buttons; persists across HTML rebuilds.
  ...
  const sync = () => {
    ...
    // Rotary knobs: reflect the live value as a CSS rotation var (--a).
  ...
  useEffect(() => {
    ...
    // Rotary knobs: vertical-drag a [data-knob] element; CSS rotates a child via var(--a).
    root.querySelectorAll<HTMLElement>('[data-knob]').forEach((el) => {
    ...
    // the keyboard's lowest note (e.g. "C3"). The keys play their note + shift.
    ...
    noteEls.forEach((el) => {
      ...
      let held: number | null = null; // actual (transposed) note while pressed
