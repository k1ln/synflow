...
export interface WebRTCInputFlowNodeData {
  id: string;
  label?: string;
  sessionId?: string;
  serverUrl?: string;
...
interface WebRTCInputFlowNodeProps {
  data: WebRTCInputFlowNodeData;
}
...
const WebRTCInputFlowNode: React.FC<WebRTCInputFlowNodeProps> = ({ data }) => {
  const eventBus = EventBus.getInstance();
  const [label, setLabel] = useState<string>(data.label || 'WebRTC In');
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(json) ? json.filter((item) => item?.config?.direction === 'capture') : [];
      setSessions(list);
      setSessionId((prev) => {
