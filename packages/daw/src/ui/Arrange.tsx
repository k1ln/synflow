...
import { Repeat, X, Drum, Music2, AudioWaveform, Volume2, VolumeX, Play, Square } from 'lucide-react';
import { songLengthSlots, type Project, type Clip, type AudioClip } from '../model/project';
...
export function Arrange({
  ...
  onToggleSongMode, onSetSongSlots, onSelectTrack, onToggleMute, onTrackVolume, onSeek, onAddClip, onRemoveClip, onToggleLoop, onClipLen, onMoveClip, onMoveAudioClip, onRemoveAudioClip, onPlayClip, previewKey,
}: {
  project: Project;
  currentStep: number;
  songMode: boolean;
  ...
  // ── Clip drag (move / resize / audio) ───────────────────────────────────────
  const drag = useRef<null | { kind: 'move' | 'resize' | 'audio'; trackId: string; clipId: string; startX: number; laneW: number; orig: number }>(null);
  const onDragMove = (e: PointerEvent) => {
    const d = drag.current; if (!d) return;
    if (d.kind === 'audio') {
      const dSteps = ((e.clientX - d.startX) / d.laneW) * totalTimelineSteps;
      onMoveAudioClip(d.trackId, d.clipId, Math.max(0, d.orig + dSteps));
      return;
    }
  ...
  const begin = (e: React.PointerEvent, kind: 'move' | 'resize' | 'audio', trackId: string, clipId: string, orig: number) => {
                ...
                <div key={track.id} className={`arr2-row ${track.id === selTrack ? 'sel' : ''} ${track.muted ? 'muted' : ''}`}>
                  <div className="arr2-trk" onClick={() => onSelectTrack(track.id)}>
                    <div className="arr2-trk-head">
                      ...
                      {track.type === 'drums' ? <Drum size={12} /> : track.type === 'audio' ? <AudioWaveform size={12} /> : <Music2 size={12} />}
                  ...
                  <div
                    className="arr2-lane"
                    style={{ ['--n' as any]: N / lineEvery, cursor: isAudio ? 'default' : 'copy' }}
                    onClick={isAudio ? undefined : (e) => { const r = e.currentTarget.getBoundingClientRect(); onAddClip(track.id, Math.floor(((e.clientX - r.left) / r.width) * N)); }}
                  >
                    {isAudio
